...
var d3 = require('d3');
var rawgraphsCore = require('@rawgraphs/rawgraphs-core');
var d3Sankey = require('d3-sankey');
var louvain = require('louvain');
var d3Gridding = require('d3-gridding');
var d3Contour = require('d3-contour');
var d3Hexbin = require('d3-hexbin');
var d3Sankey$1 = require('d3-sankey-circular');
var d3VoronoiTreemap = require('d3-voronoi-treemap');
...
var d3Sankey__namespace = /*#__PURE__*/_interopNamespace(d3Sankey$1);
...
const mapData = function (data, mapping, dataTypes, dimensions) {
  const sizeAggregator = rawgraphsCore.getDimensionAggregator('size', mapping, dataTypes, dimensions);
...
d3.transition.prototype.styles = styles; //other approach
...
function render(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const sankey = d3Sankey.sankey().nodeId(d => d.id).nodeWidth(nodesWidth).nodePadding(nodesPadding).extent([[0, 0], [chartWidth, chartHeight]]).iterations(0); // no iterations since we compute all the positions
  ...
  switch (sortNodesBy) {
    ...
    case 'name':
      ...
      .sort((a, b) => d3.ascending(typeof a.name, typeof b.name)) // then by actual value
      .sort((a, b) => d3.ascending(a.name, b.name));
      break;
  } //check the amount of space required


  let verticalSize = d3.rollups(network.nodes, v => v.length - 1, d => d.step);
  ...
  const xScale = d3.scaleBand().range([0, chartWidth - nodesWidth]).domain(mapping.steps.value).align(0).paddingInner(1); // update nodes vertical position

  d3.groups(network.nodes, d => d.step) // for each group, compute position
  .forEach(group => {
    // compute the starting point.
    let yPos0 = 0;
    const totalSize = d3.sum(group[1], d => d.y1 - d.y0) + (group[1].length - 1) * nodesPadding; // if top, do nothing.
  ...
  d3.select(svgNode).append('rect').attr('width', width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'visualization');
  ...
  link.append('path').attr('d', d3Sankey.sankeyLinkHorizontal()).attr('stroke', d => colorScale(d.source.id)).attr('stroke-width', d => Math.max(1, d.width));
...
function render$1(svgNode, data, visualOptions, mapping, originalData) {
  ...
  const sizeScale = d3.scaleSqrt().domain([0, d3.max(graph.nodes, d => d[nodeSize])]).range([minDiameter, maxDiameter]); // widthScale (for nodes)

  const widthScale = d3.scaleLinear().domain([0, d3.max(graph.links, d => d.value)]).range([0, maxDiameter]); // get the total size

  const totalValue = d3.sum(graph.nodes, d => sizeScale(d[nodeSize]) * 2); // compute padding
  ...
  d3.select(svgNode).append('rect').attr('width', width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'viz'); // draw links
...
const mapData$2 = function (data, mapping, dataTypes, dimensions) {
  // define aggregators
  const colorAggregator = rawgraphsCore.getDimensionAggregator('color', mapping, dataTypes, dimensions);
  const sizeAggregator = rawgraphsCore.getDimensionAggregator('size', mapping, dataTypes, dimensions); // add the non-compulsory dimensions.
...
function render$2(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  if (mapping.bars.dataType.type === 'date') {
    ...
    const timeFormat = d3.timeFormat(rawgraphsCore.dateFormats[mapping.bars.dataType.dateFormat]); // use it to format date
  ...
  const nestedData = d3.groups(data, d => d.series).map(d => ({
    data: d,
    totalSize: d3.sum(d[1], d => d.size)
  }));
  ...
  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background'); // set up grid

  const gridding = d3Gridding.gridding().size([width, height]).mode('grid').padding(0) // no padding, margins will be applied inside
  ...
  const barsDomain = d3.rollups(data, v => d3.sum(v, d => d.size), d => d.bars).sort(barsSortings[sortBarsBy]).map(d => d[0]); // add grid
  ...
  series.each(function (d, seriesIndex) {
    ...
    const selection = d3.select(this).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // compute each serie width and height
    ...
    const barScale = d3.scaleBand().range([0, horizontalBars ? seriesHeight : seriesWidth]).domain(barsDomain) //convert padding from px to percentage
    .padding(padding / ((horizontalBars ? seriesHeight : seriesWidth) / barsDomain.length));
    const seriesDomain = d3.extent(d.data[1], d => d.size);
    const sizeScale = d3.scaleLinear().domain(useSameScale ? sizeDomain : seriesDomain).nice().range(horizontalBars ? [0, seriesWidth] : [seriesHeight, 0]);
    ...
    if (horizontalBars) {
      selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + seriesHeight + ')').call(d3.axisBottom(sizeScale)).call(g => g.append('text').attr('font-family', 'Arial, sans-serif').attr('font-size', 10).attr('x', seriesWidth).attr('dy', -5).attr('fill', 'black').attr('font-weight', 'bold').attr('text-anchor', 'end').attr('display', seriesIndex === 0 || repeatAxesLabels ? null : 'none').text(d => {
        return mapping['size'].value ? `${mapping['size'].value} [${mapping.size.config.aggregation}]` : '';
      }));
      selection.append('g').attr('id', 'yAxis').attr('transform', 'translate(' + sizeScale(0) + ',0)').call(d3.axisLeft(barScale).tickSizeOuter(0)).call(g => g.append('text').attr('font-family', 'Arial, sans-serif').attr('font-size', 10).attr('x', 4).attr('fill', 'black').attr('font-weight', 'bold').attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').attr('display', seriesIndex === 0 || repeatAxesLabels ? null : 'none').text(mapping['bars'].value));
    } else {
      const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(d3.axisBottom(barScale).tickSizeOuter(0)).call(g => g.append('text').attr('x', seriesWidth).attr('y', -4).attr('text-anchor', 'end').attr('display', seriesIndex === 0 || repeatAxesLabels ? null : 'none').text(mapping['bars'].value).styles(styles.axisLabel));

      if (rotation !== 0) {
        xAxis.selectAll('.tick text').attr('transform', `rotate(${-rotation})`).attr('text-anchor', 'end').attr('dx', '-0.8em').attr('dy', '0.15em');
      }

      selection.append('g').attr('id', 'yAxis').call(d3.axisLeft(sizeScale)).call(g => g.append('text').attr('x', 4).attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').attr('display', seriesIndex === 0 || repeatAxesLabels ? null : 'none').text(d => {
        return mapping['size'].value ? `${mapping['size'].value} [${mapping.size.config.aggregation}]` : '';
      }).styles(styles.axisLabel));
    }

    if (showSeriesLabels) {
      d3.select(this).append('text').text(d => d.data[0]).attr('y', 4).attr('x', 4).styles(styles.seriesLabel);
    }
  }); // add legend

  if (showLegend) {
    const legendLayer = d3.select(svgNode).append('g').attr('id', 'legend').attr('transform', `translate(${width},${marginTop})`);
    const chartLegend = rawgraphsCore.legend().legendWidth(legendWidth);
...
const mapData$3 = function (data, mapping, dataTypes, dimensions) {
  ...
  const barsAggregators = rawgraphsCore.getDimensionAggregator('bars', mapping, dataTypes, dimensions);
  let results = [];
  d3.rollups(data, v => {
...
function render$3(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  if (mapping.groups.dataType.type === 'date') {
    // set date format  from input data
    const timeFormat = d3.timeFormat(rawgraphsCore.dateFormats[mapping.groups.dataType.dateFormat]); // use it to format date
  ...
  const nestedData = d3.rollups(data, v => v, d => d.series).map(d => ({
    data: d,
    totalSize: d3.sum(d[1], d => d.size)
  })); // sort series
  ...
  const setsDomain = d3.rollups(data, v => d3.sum(v, d => d.size), d => d.groups).sort(stacksSortings[SortXAxisBy]).map(d => d[0]);
  ...
  series.each(function (d, serieIndex) {
    ...
    const setScale = d3.scaleBand().range([0, serieWidth]).domain(setsDomain).padding(setsPadding / (serieWidth / setsDomain.length) //convert padding from px to percentage
    );
    const barScale = d3.scaleBand().range([0, setScale.bandwidth()]).domain(barsDomain).padding(barsPadding / (setScale.bandwidth() / barsDomain.length) //convert padding from px to percentage
    );
    const localDomain = d3.extent(d.data[1], e => e.size);
    const sizeScale = d3.scaleLinear().domain(useSameScale ? sizeDomain : localDomain).nice().range([serieHeight, 0]); // check if padding is too high and leave no space for bars
    ...
    const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(d3.axisBottom(setScale).tickSizeOuter(0));

    if (rotation !== 0) {
      xAxis.selectAll('.tick text').attr('transform', `rotate(${-rotation})`).attr('text-anchor', 'end').attr('dx', '-0.8em').attr('dy', '0.15em');
    }

    selection.append('g').attr('id', 'yAxis').call(d3.axisLeft(sizeScale).tickSizeOuter(0));

    if (showSeriesLabels) {
      d3.select(this).append('text').attr('x', 4).attr('y', 4).text(d => d.data[0]).styles(styles.seriesLabel);
...
function render$4(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  if (mapping.stacks.dataType.type === 'date') {
    // set date format  from input data
    const timeFormat = d3.timeFormat(rawgraphsCore.dateFormats[mapping.stacks.dataType.dateFormat]); // use it to format date
  ...
  const scaleRollup = d3.rollups(data, v => d3.sum(v, d => d.size), d => d.stacks + '_' + d.series).map(d => d[1]);
  let sizeDomain = [0, d3.max(scaleRollup)]; // stacks (x axis) sorting functions
  ...
  const stacksDomain = d3.rollups(data, v => d3.sum(v, d => d.size), d => d.stacks).sort(stacksSortings[SortXAxisBy]).map(d => d[0]);
  const barsDomain = [...new Set(data.map(d => d.bars))]; // add grid
  ...
  series.each(function (d, serieIndex) {
    ...
    let localStack = Array.from(d3.rollup(d.data[1], ([e]) => e, e => e.stacks, e => e.bars)); // creaet an object with ordering methods
    ...
    let stack = d3.stack().keys(barsDomain).value((data, key) => data[1].has(key) ? data[1].get(key).size : 0).order(d3__namespace[orderings[stacksOrder]]);
    let stackedData = stack(localStack); // check if padding is too high and leave no space for bars
    ...
    const stacksScale = d3.scaleBand().range([0, serieWidth]).domain(stacksDomain).padding(stacksPadding / (serieWidth / stacksDomain.length) //convert padding from px to percentage
    );
    let localDomain = [0, d3.max(d3.rollups(d.data[1], v => d3.sum(v, d => d.size), d => d.stacks).map(d => d[1]))];
    const sizeScale = d3.scaleLinear().domain(useSameScale ? sizeDomain : localDomain).nice().range([serieHeight, 0]);
    ...
    const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(d3.axisBottom(stacksScale).tickSizeOuter(0));
...
function render$5(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const grouped = d3.groups(data, d => d.series); // reduce them for sorting
  ...
  const sizeScale = d3.scaleSqrt().domain(d3.extent(data, d => d.size)).range([minDiameter / 2, maxDiameter / 2]); // create y scale

  const yScale = d3.scaleBand().rangeRound([0, chartHeight]).domain(grouped.map(d => d[0])).align(0.5).padding(0); // prepare data with initial vales, so the simulation won't start from 0,0
  ...
  let simulation = d3.forceSimulation(data).force('x', d3.forceX().x(d => xScale(d.xValue))).force('y', d3.forceY(d => yScale(d.series) + yScale.bandwidth() / 2)).force('collision', d3.forceCollide().radius(d => sizeScale(d.size) + nodePadding)); // add background

  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background'); // get svg node

  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'viz');

  const xAxis = g => {
    return g.attr('transform', `translate(0,${chartHeight})`).call(d3.axisBottom(xScale)).call(g => g.append('text').attr('x', chartWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['xValue'].value).styles(styles.axisLabel));
  };

  const yAxis = g => {
    return g.call(d3.axisLeft(yScale).tickSize(Math.round(-chartWidth)).tickSizeOuter(0)).call(g => g.selectAll('line').styles(styles.axisLine)).call(g => g.append('text').attr('x', 4).attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').text(mapping['series'].value).styles(styles.axisLabel));
  ...
  labelsLayer.selectAll('g').data(d => mapping.label.value ? d[1] : []).join('g').attr('id', d => Array.isArray(d.label) ? d.label.toString() : d.label).attr('transform', d => `translate(${d.x},${d.y})`).append('text').attr('x', 0).attr('y', 0).attr('text-anchor', 'middle').attr('dominant-baseline', 'text-before-edge').selectAll('tspan').data(d => Array.isArray(d.label) ? d.label : [d.label]).join('tspan').attr('x', 0).attr('y', 0).attr('dy', (d, i) => i * 12).text((d, i) => {
    if (d && mapping.label.dataType[i].type === 'date') {
      return d3.timeFormat(rawgraphsCore.dateFormats[mapping.label.dataType[i].dateFormat])(d);
    } else {
      return d;
    }
  ...
  if (autoHideLabels) {
    rawgraphsCore.labelsOcclusion(labelsLayer.selectAll('text'), d => d.size);
  }
...
const mapData$6 = function (data, mapping, dataTypes, dimensions) {
  ...
  const colorAggregator = rawgraphsCore.getDimensionAggregator('color', mapping, dataTypes, dimensions); // add the non-compulsory dimensions.
...
function render$6(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const rotation = Number(xAxisLabelRotation); // Margin convention
  ...
  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'viz');
  const yDomain = yOrigin ? [0, d3.max(data, d => d.value)] : d3.extent(data, d => d.value);
  const yScale = d3.scaleLinear().domain(yDomain).nice().range([chartHeight, 0]);
  const groupsWithMedian = d3.rollups(data, v => d3.median(v, d => d.value), d => d.group);
  const groupSortings = {
  ...
  const xScale = d3.scalePoint().rangeRound([0, chartWidth]).domain(groupsDomain).padding(0.5); // if series is exposed, recreate the nested structure

  const nestedData = d3.rollups(data, v => {
  ...
  svg.append('g').attr('id', 'y axis').call(d3.axisLeft(yScale)).append('text').styles(styles.axisLabel).attr('x', 4).attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').text(mapping['value'].value);
  const xAxisG = svg.append('g').attr('id', 'x axis').attr('transform', 'translate(0,' + chartHeight + ')').call(d3.axisBottom(xScale));

  if (rotation !== 0) {
    xAxisG.selectAll('.tick text').attr('transform', `rotate(${-rotation})`).attr('text-anchor', 'end').attr('dx', '-0.8em').attr('dy', '0.15em');
  } //append boxplots
...
const visualOptions$6 = {
  ...
  SortXAxisBy: {
    ...
    label: 'Sort X axis by',
    group: 'chart',
    options: ['Name', 'Median (descending)', 'Median (ascending)', 'Original'],
    default: 'Name'
  },
  xAxisLabelRotation: {
    type: 'text',
...
function render$7(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const nestedData = d3.groups(data, d => d.series); // comupte max values for series
  ...
  const svg = d3.select(svgNode); // add background
  ...
  const gridding = d3Gridding.gridding().size([width, height]).mode('grid').padding(0) // no padding, margins will be applied inside
  .cols(columnsNumber);
  const griddingData = gridding(nestedData); // draw the grid if asked
  ...
  const sizeScale = d3.scaleSqrt().domain([0, d3.max(data, d => d.size)]).rangeRound([0, maxRadius]);
  /*
  ...
  */
  ...
  series.each(function (serie, seriesIndex) {
    // make a local selection for each serie
    const selection = d3.select(this).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // add the clip path

    selection.attr('clip-path', 'url(#serieClipPath)'); // add series titles

    if (showSeriesLabels) {
      d3.select(this).append('text').attr('y', 4).attr('x', 4).text(d => d[0]).styles(styles.seriesLabel);
    ...
    d3.extent(data, e => e.y) : // compute extent of the single serie
    ...
    d3.extent(data, e => e.x) : // compute extent of the single serie
    ...
    const xScale = mapping.x.dataType.type === 'date' ? d3.scaleTime() : d3.scaleLinear();
    xScale.domain(xDomain).rangeRound([0, seriesWidth]).nice(); // y scale

    const yScale = mapping.y.dataType.type === 'date' ? d3.scaleTime() : d3.scaleLinear();
    yScale.domain(yDomain).rangeRound([seriesHeight, 0]).nice(); // create axis functions
    // x axis

    const xAxis = g => {
      return g.attr('transform', `translate(0,${seriesHeight})`).call(d3.axisBottom(xScale)).call(g => g.append('text').attr('x', seriesWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['x'].value).styles(styles.axisLabel));
    }; // y axis


    const yAxis = g => {
      return g.call(d3.axisLeft(yScale)).call(g => g.append('text').attr('x', 4).attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').text(mapping['y'].value).styles(styles.axisLabel));
  ...
  if (showLegend) {
    ...
    if (mapping.size.value) {
      ...
      legendSizeScale.domain(d3.extent(data, d => d.size)).rangeRound([sizeScale(d3.min(data, d => d.size)), maxRadius]);
...
const mapData$8 = function (data, mapping, dataTypes, dimensions) {
  const sizeAggregator = rawgraphsCore.getDimensionAggregator('size', mapping, dataTypes, dimensions);
  const rankAggregator = rawgraphsCore.getDimensionAggregator('rank', mapping, dataTypes, dimensions);
  let results = [];
  d3.rollups(data, v => d3.rollups(v, vv => {
    const item = {
...
function render$8(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  let stack = d3.stack().keys(streamsDomain).value((data, key) => data[1].has(key) ? data[1].get(key).size : 0).order(d3__namespace[streamsOrder]).offset(d3__namespace[streamsOffset]); // create nest structure

  const nestedData = d3.rollups(data, v => {
    let localStack = Array.from(d3.rollup(v.sort((a, b) => d3.ascending(a.x, b.x)), // check that x axis is properly sorted
    ([e]) => e, e => e.x, e => e.streams));
    let stackedData = stack(localStack); // re-sort streams
    ...
    stackedData[0].map((row, rowIndex) => {
      ...
      let minValue = d3.min(vStack, d => d[0]); // keep original order
    ...
    data: d,
    totalSize: d3.sum(d[1].flat(), e => e[1] - e[0]),
  ...
  const globalDomain = [d3.min(stacksValues, d => d[0]), d3.max(stacksValues, d => d[1])]; // x scale
  ...
  series.each(function (d, serieIndex) {
    ...
    stackedData[0].map((row, rowIndex) => {
      ...
      let index = 0;
      vStack.sort((a, b) => d3.ascending(a.rankIndex, b.rankIndex)).forEach(e => {
    ...
    let localDomain = [d3.min(stackedData, d => d3.min(d, d => d[0])), d3.max(stackedData, d => d3.max(d, d => d[1]))];
    const sizeScale = d3.scaleLinear().domain(useSameScale ? globalDomain : localDomain).nice().range([serieHeight, 0]);
    selection.append('g').selectAll('path').data(stackedData).join('path').attr('fill', ({
      key
    }) => {
      return colorScale(key);
    }).attr('d', d3.area().curve(d3__namespace[interpolation]).x(d => xScale(d.data[0])).y0(d => sizeScale(d[0]) - d.padding).y1(d => sizeScale(d[1]) - d.padding)).append('title').text(({
      key
    }) => key);
    const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + serieHeight + ')').call(d3.axisBottom(xScale).tickSizeOuter(0));
    ...
    if (showLabels) {
      ...
      if (labelsType == 'On path') {
        ...
        defs.selectAll('path').data(stackedData.filter(d => d3.sum(d, e => e[1] - e[0]) > 0)).join('path').attr('id', (d, i) => 'path-' + serieIndex + '-' + i).attr('d', d3.line().curve(d3__namespace[interpolation]).x(d => xScale(d.data[0])).y(d => sizeScale((d[0] + d[1]) / 2) - d.padding));
        selection.append('g').attr('id', 'labels').selectAll('text').data(stackedData.filter(d => d3.sum(d, e => e[1] - e[0]) > 0)).join('text').attr('dy', '0.5ex').attr('class', 'label').append('textPath').attr('xlink:xlink:href', (d, i) => '#path-' + serieIndex + '-' + i).attr('startOffset', d => {
          // find max value
          const maxIndex = d3.maxIndex(d, e => e[1] - e[0]); // get x position

          d.offset = Math.round(maxIndex / d.length * 100); //clamp offset between 5% and 95%, return it
      ...
      if (labelsType == 'On point') {
        let labels = selection.append('g').attr('id', 'labels').selectAll('text').data(stackedData.filter(d => d3.sum(d, e => e[1] - e[0]) > 0)).join('text').attr('x', d => {
          // find max value index
          const maxIndex = d3.maxIndex(d, e => e[1] - e[0]);
          d.maxElement = d[maxIndex]; // get x position
...
const mapData$9 = function (data, mapping, dataTypes, dimensions) {
  const colorAggregator = rawgraphsCore.getDimensionAggregator('color', mapping, dataTypes, dimensions);
  const sizeAggregator = rawgraphsCore.getDimensionAggregator('size', mapping, dataTypes, dimensions);
  const labelAggregators = rawgraphsCore.getDimensionAggregator('label', mapping, dataTypes, dimensions); // add the non-compulsory dimensions.
...
function render$9(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const hierarchy = d3.hierarchy(nest).sum(d => d[1] instanceof Map ? 0 : d[1].size) // since maps have a .size porperty in native javascript, sum only values for leaves, and not for Maps
  .sort((a, b) => {
    if (sortCirclesBy !== 'original') {
      return d3__namespace[sortCirclesBy](a.value, b.value);
    }
  });

  const pack = data => d3.pack().size([chartWidth, chartHeight]).padding(showHierarchyLabels ? padding + 4 : padding)(hierarchy);

  const root = pack();
  const circle = d3.arc().innerRadius(0).outerRadius(d => d).startAngle(-Math.PI).endAngle(Math.PI); // add background
  ...
  labelsLayer.selectAll('g').data(mapping.label.value ? root.leaves() : []).join('g').attr('transform', d => `translate(${d.x + 1},${d.y + 1})`).append('text').attr('text-anchor', 'middle').attr('dominant-baseline', 'text-before-edge').selectAll('tspan').data((d, i, a) => {
    ...
    if (d && mapping.label.dataType[i].type === 'date') {
      return d3.timeFormat(dateFormats[mapping.label.dataType[i].dateFormat])(d);
    } else {
      return d;
    }
  }).styles((d, i) => styles[labelStyles[i]]);
  ...
  if (autoHideLabels) {
    ...
    rawgraphsCore.labelsOcclusion(labelsLayer.selectAll('text'), d => d.r);
  }
  ...
  if (showLegend) {
    ...
    let sizeScale = d3.scaleSqrt().domain(d3.extent(hierarchy.leaves(), d => d.value)).rangeRound(d3.extent(hierarchy.leaves(), d => d.r)); // if the maximum radius is bigger than a quarter of the legend width,
...
const mapData$a = function (data, mapping, dataTypes, dimensions) {
  const colorAggregator = rawgraphsCore.getDimensionAggregator('color', mapping, dataTypes, dimensions);
  const sizeAggregator = rawgraphsCore.getDimensionAggregator('size', mapping, dataTypes, dimensions);
  const labelAggregators = rawgraphsCore.getDimensionAggregator('label', mapping, dataTypes, dimensions);
  const results = [];
  d3.rollups(data, v => {
    const item = {
      hierarchy: new Map(mapping.hierarchy.value.map(d => [d, v[0][d]])),
...
function render$a(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const nest = d3.rollup(data, v => v[0], ...mapping.hierarchy.value.map(level => d => d.hierarchy.get(level)));
  const hierarchy = d3.hierarchy(nest) // since maps have also a .size porperty, sum only values for leaves, and not for Maps
  ...
  hierarchy.descendants().filter(d => d.data[0] === '') // select nodes with empty key
  .forEach(d => {
  ...
  const sizeScale = d3.scaleSqrt().domain([0, d3.max(hierarchy.leaves(), d => d.value)]).range([0, maxDiameter / 2]); // get the total size

  const totalValue = d3.sum(hierarchy.leaves(), d => sizeScale(d.value) * 2); // compute padding

  const padding = (circumference - totalValue) / (hierarchy.leaves().length - 1); // dictionary to choose algorithm according to options
  ...
  const tree = nest => {
    ...
    .size([2 * Math.PI, radius - 100]).separation((a, b) => sizeScale(a.value) + sizeScale(b.value) + padding)(hierarchy);
  };

  const root = tree(); // add background

  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')').attr('id', 'viz');
  svg.append('g').attr('id', 'links').selectAll('path').data(root.links()).join('path').attr('d', d3.linkRadial().angle(d => d.x).radius(d => d.y)).attr('fill', 'none').attr('stroke', '#ccc');
...
function colorDomain(data, mapping, visualOptions) {
  ...
  const contours = d3Contour.contourDensity().x(d => x(d.x)).y(d => y(d.y)).size([chartWidth, chartHeight]).bandwidth(bandwidth)(data);
...
function render$c(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const xDomain = xOrigin ? [0, d3.max(data, d => d.x)] : d3.extent(data, d => d.x);
  const x = mapping.x.dataType.type === 'date' ? d3.scaleTime() : d3.scaleLinear();
  x.domain(xDomain).rangeRound([0, chartWidth]).nice(); // y scale

  const yDomain = yOrigin ? [0, d3.max(data, d => d.y)] : d3.extent(data, d => d.y);
  const y = mapping.y.dataType.type === 'date' ? d3.scaleTime() : d3.scaleLinear();
  y.domain(yDomain).rangeRound([chartHeight, 0]).nice();

  const xAxis = g => {
    return g.attr('transform', `translate(0,${chartHeight})`).call(d3.axisBottom(x)).call(g => g.append('text').attr('x', chartWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['x'].value).styles(styles.axisLabel));
  };

  const yAxis = g => {
    return g.call(d3.axisLeft(y)).call(g => g.append('text').attr('x', 4).attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').text(mapping['y'].value).styles(styles.axisLabel));
  };
  ...
  contourBand.append('path').attr('fill', d => colorScale(d.value)).attr('stroke', d => d3.lab(colorScale(d.value)).darker(1)).attr('stroke-opacity', (d, i) => i % labelThresholds === 0 ? 1 : 0.3).attr('d', d3.geoPath());
  ...
  if (autoHideLabels) {
    rawgraphsCore.labelsOcclusion(labelsLayer.selectAll('text'), (d, i) => i);
  }
...
function render$d(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'visualization');
  const vizLayer = svg.append('g').attr('id', 'viz');
  const hullsData = d3.groups(data, d => d.group && d.group.length ? d.group : null);
...
function render$e(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const padding = (chartHeight - totalValue) / (hierarchy.leaves().length - 1); // dictionary to choose algorithm according to options
  ...
  const tree = nest => {
    return layouts[layout] // compute according to the options
    .size([chartHeight, chartWidth]).separation((a, b) => sizeScale(a.value) + sizeScale(b.value) + padding)(hierarchy);
  };
  ...
  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'viz');
  svg.append('g').attr('id', 'links').selectAll('path').data(root.links()).join('path').attr('d', d3.linkHorizontal().x(d => d.y).y(d => d.x)).attr('fill', 'none').attr('stroke', '#ccc');
...
function render$f(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  groups.sort((a, b) => {
    ...
    if (sortGroupsBy === 'group') {
      ...
      return d3__namespace[sortGroupsBy](a[1][0].startDate, b[1][0].startDate);
    }
  }); // x scale

  const xMin = d3.min(data, d => d.startDate);
  const xMax = d3.max(data, d => d.endDate);
  const x = mapping.startDate.dataType.type === 'date' ? d3.scaleTime() : d3.scaleLinear();
  x.domain([xMin, xMax]).rangeRound([0, chartWidth]).nice();
  const lines = d3.sum(groups, g => d3.max(g[1], d => d.level + 1));
  const heightScale = d3.scaleBand().domain(d3.range(lines)).range([0, chartHeight]).paddingInner(barPadding).paddingOuter(barPadding / 2).align(1);
  const lineHeight = heightScale.bandwidth();
  const lineStep = heightScale.step();

  const xAxis = g => {
    return g.attr('transform', `translate(0,${chartHeight})`).call(d3.axisBottom(x));
  };
  ...
  const axisLayer = svg.append('g').attr('id', 'axis');
  axisLayer.append('g').call(xAxis);
  let stack = d3.stack().keys(groups.map(d => d[0])).value((d, key) => {
    const a = d.filter(f => f[0] === key)[0];
    return d3.max(a[1].map(c => c.level)) + 1;
  });
  const vizLayer = svg.append('g').attr('id', 'viz');
  const groupsG = vizLayer.selectAll('g').data(stack([groups])).join('g').attr('id', d => d.key).attr('transform', d => `translate(0,${d[0][0] * lineStep})`);
  ...
  groupsG.selectAll('rect').data((d, i) => d[0].data[d.index][1]).join('rect').attr('x', d => x(d.startDate)).attr('y', d => lineStep * d.level + (lineStep - lineHeight) / 2).attr('width', d => d3.max([1, x(d.endDate) - x(d.startDate)])).attr('height', lineHeight).attr('fill', d => colorScale(d.color));
...
function colorDomain$1(data, mapping, visualOptions) {
  ...
  const y = mapping.y.dataType === 'date' ? d3.scaleTime() : d3.scaleLinear();
  y.domain(yDomain).rangeRound([chartHeight, 0]).nice();
  const hexbin = d3Hexbin.hexbin().x(d => x(d.x)).y(d => y(d.y)).radius(diameter / 2).extent([[margin.left, margin.top], [chartWidth, chartHeight]]);
...
function render$g(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const hexbin = d3Hexbin.hexbin().x(d => x(d.x)).y(d => y(d.y)).radius(diameter / 2).extent([[margin.left, margin.top], [chartWidth, chartHeight]]);
  const bins = hexbin(data);
  const size = d3.scaleSqrt().domain([0, d3.max(bins, d => d.length)]).rangeRound([weightSize ? 0 : diameter / 2, diameter / 2]);
  const hex = vizLayer.selectAll('g').data(bins).join('g');
  hex.append('path').attr('d', d => hexbin.hexagon(size(d.length))).attr('transform', d => `translate(${d.x},${d.y})`).attr('fill', d => colorScale(d.length)).attr('stroke', 'white');
  ...
  if (autoHideLabels) {
    rawgraphsCore.labelsOcclusion(hex.selectAll('text'), d => d.length);
  }
...
const mapData$h = function (data, mapping, dataTypes, dimensions) {
  const yAggregator = rawgraphsCore.getDimensionAggregator('y', mapping, dataTypes, dimensions);
  let results = [];
  d3.rollups(data, v => {
    const item = {
...
function render$h(node, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = d3.select(node); // nest data

  let nestedData = d3.groups(data, d => d.group);
  const chartWidth = width - margin.left - margin.right;
  const chartHeight = height - margin.top - margin.bottom;
  const groupsDomain = nestedData.map(d => d[0]);
  const groupsScale = d3.scaleBand().range([0, chartHeight]).domain(groupsDomain).paddingOuter(0).paddingInner(padding / (chartWidth / groupsDomain.length)); // x scale

  const xDomain = d3.extent(data, e => e.x);
  const xScale = mapping.x.dataType.type === 'date' ? d3.scaleTime() : d3.scaleLinear();
  xScale.domain(xDomain).range([0, chartWidth]); // y scale

  const yDomain = [0, d3.max([-d3.min(data, e => e.y), d3.max(data, e => e.y)]) / bands];
  const yScale = d3.scaleLinear().domain(yDomain).range([groupsScale.bandwidth(), 0]); // area function

  let area = d3.area().curve(d3__namespace[interpolation]).x(d => xScale(d.x)).y0(groupsScale.bandwidth()).y1(d => yScale(d.y)); // create the clip path
  ...
  const xAxis = g => {
    return g.attr('transform', `translate(0,${chartHeight})`).call(d3.axisBottom(xScale).tickSizeOuter(0)).call(g => g.append('text').attr('x', chartWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['x'].value).styles(styles.axisLabel));
  };
...
const mapData$i = function (data, mapping, dataTypes, dimensions) {
  const colorAggregator = rawgraphsCore.getDimensionAggregator('color', mapping, dataTypes, dimensions);
  const yAggregator = rawgraphsCore.getDimensionAggregator('y', mapping, dataTypes, dimensions); // add the non-compulsory dimensions.
...
function render$i(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const nestedData = d3.rollups(data, v => v.sort((a, b) => d3.ascending(a.x, b.x)), d => d.series, d => d.lines); // comupte max values for series
  // will add it as property to each series.
  ...
  .cols(mapping.series.value ? columnsNumber : 1);
  const griddingData = gridding(nestedData);
  const svg = d3.select(svgNode).append('g').attr('id', 'viz'); // create the clip path
  ...
  series.each(function (d, serieIndex) {
    ...
    const yScale = d3.scaleLinear().domain(yDomain).nice().range([serieHeight, 0]); // define the line path function

    const line = d3.line().x(function (d) {
      return xScale(d.x);
    }).y(function (d) {
      return yScale(d.y);
    }).curve(d3__namespace[interpolation]);
    ...
    groups.append('path').attr('d', d => line(d[1])).attr('stroke', d => colorScale(d[1][0].color)).attr('fill', 'none');
...
function render$j(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  let rowsValues = d3.rollups(data, v => d3.sum(v, d => d.size), d => d.y).map(d => ({
    key: d[0],
    value: d[1]
  }));
  const colsValues = d3.rollups(data, v => d3.sum(v, d => d.size), d => d.x).map(d => ({
    key: d[0],
    value: d[1]
  }));
  ...
  const x = d3.scaleBand().range([0, chartWidth]).domain(cols).padding(padding / cellSize); // d3 expects padding expressed in % (0-1)

  const y = d3.scaleBand().range([0, chartHeight]).domain(rows).padding(padding / cellSize); // d3 expects padding expressed in % (0-1)

  const sizeScale = d3.scaleSqrt().domain([0, d3.max(data, d => d.size)]).range([0, cellSize]);
  const roundingScale = d3.scaleLinear().domain([0, 100]).rangeRound([0, 50]); // add background
  ...
  if (showGrid) {
    ...
    .call(d3.axisTop(x).tickSize(Math.round(-chartHeight + 1)).tickSizeOuter(0)); // add the Y gridlines

    svg.append('g').attr('class', 'grid').attr('transform', 'translate(0,' + (cellSize / 2 + 1) + ')') // not clear why there is an offset of 2px
    .call(d3.axisLeft(y).tickSize(Math.round(-chartWidth + 1)).tickSizeOuter(0));
  } // add top x axis


  svg.append('g').call(d3.axisTop(x).tickSizeOuter(0)).selectAll('text').attr('dx', Math.sqrt(12)) // proportional to text size. @TODO we should use a variable.
  ...
  svg.append('g').call(d3.axisLeft(y).tickSizeOuter(0)).selectAll('text'); // add y axis title
  ...
  if (showLegend) {
    ...
    if (mapping.size.value) {
      const legendSizeScale = sizeScale.copy();
      const maxSize = sizeScale(d3.max(data, d => d.size)) > legendWidth * 0.66 ? legendWidth * 0.66 : sizeScale(d3.max(data, d => d.size));
      const shape = rounding >= 100 ? 'circle' : 'square';
      legendSizeScale.domain(d3.extent(data, d => d.size)).rangeRound([sizeScale(d3.min(data, d => d.size)), maxSize]);
...
function render$k(node, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = d3.select(node); // add background
  ...
  const axesScales = new Map(Array.from(keys, (key, i) => [key, d3.scaleLinear(d3.extent(data, d => d.dimensions[i]), [0, horizontal ? chartWidth : chartHeight])]));
  const catScales = d3.scalePoint(keys, [0, horizontal ? chartHeight : chartWidth]);
  const line = d3.line();
  ...
  viz.append('g').attr('id', 'lines').attr('fill', 'none').selectAll('path').data(data).join('path').attr('d', d => line(d.dimensions)).attr('stroke', d => colorScale(d.color)).attr('stroke-width', strokeWidth).attr('stroke-opacity', strokeOpacity); // add axes
...
const mapData$l = function (data, mapping, dataTypes, dimensions) {
  ...
  // as we are working on a multiple dimension (bars), `getDimensionAggregator` will return an array of aggregator functions
  // the order of aggregators is the same as the value of the mapping
  const arcsAggregators = rawgraphsCore.getDimensionAggregator('arcs', mapping, dataTypes, dimensions); // we will use rollup to populate a flat array of objects
  // that will be passed to the render
...
function render$l(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const svg = d3.select(svgNode).attr('width', showLegend ? width + legendWidth : width).attr('height', height); // add background
  ...
  const maxRadius = d3.min([griddingData[0].width - margin.right - margin.left, griddingData[0].height - margin.top - margin.bottom]) / 2; //create size scale

  const sizeScale = d3.scaleSqrt().domain([0, d3.max(data, d => d.totalValue)]).range([0, maxRadius]); // add grid
  ...
  series.each(function (d, seriesIndex) {
    // make a local selection for each serie
    const selection = d3.select(this); // compute each serie width and height
    ...
    let angles = d3.pie().sort(null)(arcs); // compuet biggest radius

    let radius = sizeScale(d.totalValue);
    let arc = d3.arc().innerRadius(drawDonut && sizeScale(d.totalValue) > arcTichkness ? sizeScale(d.totalValue) - arcTichkness : 0).outerRadius(radius);
    let pie = selection.append('g').attr('id', 'pie').attr('transform', 'translate(' + (margin.left + seriesWidth / 2) + ',' + (margin.top + seriesHeight / 2) + ')');
    pie.selectAll('path').data(angles).join('path').attr('fill', (d, i) => {
      return colorScale(arcsSize[i].name);
    }).attr('stroke', background).attr('d', e => arc.startAngle(e.startAngle).endAngle(e.endAngle)()); // add arcs labels

    if (showArcValues) {
      let pieLabels = pie.append('g').attr('id', 'labels').selectAll('g').data(angles).join('g').attr('transform', e => {
        return `translate(${arc.startAngle(e.startAngle).endAngle(e.endAngle).centroid(e)})`;
      });
  ...
  if (showLegend) {
    ...
    if (mapping.series.value) {
      const legendSizeScale = sizeScale.copy();
      legendSizeScale.domain(d3.extent(data, d => d.totalValue)).rangeRound([sizeScale(d3.min(data, d => d.totalValue)), maxRadius]);
...
const mapData$m = function (data, mapping, dataTypes, dimensions) {
  // define aggregators
  rawgraphsCore.getDimensionAggregator('color', mapping, dataTypes, dimensions); // we will use rollup to populate a flat array of objects
  // that will be passed to the render
...
function render$m(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const nestedData = d3.rollups(data, v => v, d => d.series, d => d.name).map(d => {
  ...
  const gridding = d3Gridding.gridding().size([width, height]).mode('grid').padding(0) // no padding, margins will be applied inside
  .cols(mapping.series.value ? columnsNumber : 1);
  const griddingData = gridding(nestedData); // create the clip path
  ...
  const radialScale = d3.scalePoint().domain(axesDomain).range([-Math.PI / 2, Math.PI * 1.5]) // starts from -PI/2 (upper part of circle)
  .padding(0.5) // calculate half padding at beginning and half at end
  .align(0) // put all the apdding at the end
  .round(false);
  const maxValue = d3.max(data, d => d.value);
  const innerRadius = innerDiameter / 2;
  const outerRadius = d3.min([(griddingData[0].width - margin.right - margin.left) / 2, (griddingData[0].height - margin.top - margin.bottom) / 2]);
  const axesScale = d3.scaleLinear().domain([0, maxValue]).nice().rangeRound([innerRadius, outerRadius]);
  const axesGrid = d3.scaleLinear().domain([maxValue, 0]).rangeRound([innerRadius, outerRadius]); // add grid
  ...
  */
  ...
  series.each(function (d, seriesIndex) {
    // make a local selection for each serie
    const selection = d3.select(this); // apply clipPath
    ...
    let axisFunction = d3.axisLeft(axesGrid); // add a circle for each tick on the axis
    ...
    let radarLine = d3.lineRadial().curve(curveType[interpolation]).radius(d => axesScale(d.value)).angle(d => radialScale(d.axes) + Math.PI / 2);
...
function render$n(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const sankey = d3Sankey$1.sankeyCircular().nodeId(d => d.id).nodeAlign(d3Sankey__namespace[alignments[alignment]]).nodeWidth(nodesWidth).nodePadding(nodesPadding).circularLinkGap(circularLinkGap).extent([[0, 0], [chartWidth, chartHeight]]).iterations(iterations);
  const network = sankey({
    nodes,
    links
  }); // add background
...
function render$o(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const nestedData = d3.groups(data, d => d.series); // select the SVG element
  ...
  let xScale = d3.scalePoint().domain(columns).range([margin.left, griddingData[0].width - margin.right]);
  let yGlobalScale = d3.scaleLinear().domain(d3.extent(data.flatMap(d => [d.source, d.target]))).range([griddingData[0].height - margin.bottom, margin.top]);
  /*
      YOU CAN PUT HERE CODE THAT APPLIES TO ALL THE SERIES
    */
  // do stuff for each serie
  ...
  series.each(function (d, seriesIndex) {
    ...
    d3.forceSimulation(labels).force('y', d3.forceY().y(d => d.y)).force('x', d3.forceX().x(d => d.x).strength(1)).force('collision', d3.forceCollide().radius(nonOverlap)).stop().tick(150); // precalculate positions
    // add labels
...
function render$p(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, serieIndex) {
    ...
    selection.append('g').selectAll('path').data(stackedData).join('path').attr('fill', ({
      ...
      return colorScale(key);
    }).attr('d', d3.area().curve(d3__namespace[interpolation]).x(d => xScale(d.data[0])).y0(d => sizeScale(d[0])).y1(d => sizeScale(d[1]))).append('title').text(({
      key
    }) => key);
    const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + serieHeight + ')').call(d3.axisBottom(xScale).tickSizeOuter(0));
    ...
    if (showLabels) {
      ...
      if (labelsType == 'On path') {
        let defs = d3.select(svgNode).append('defs');
        defs.selectAll('path').data(stackedData.filter(d => d3.sum(d, e => e[1] - e[0]) > 0)).join('path').attr('id', (d, i) => 'path-' + serieIndex + '-' + i).attr('d', d3.line().curve(d3__namespace[interpolation]).x(d => xScale(d.data[0])).y(d => sizeScale((d[0] + d[1]) / 2)));
        selection.append('g').attr('id', 'labels').selectAll('text').data(stackedData.filter(d => d3.sum(d, e => e[1] - e[0]) > 0)).join('text').attr('dy', '0.5ex').attr('class', 'label').append('textPath').attr('xlink:xlink:href', (d, i) => '#path-' + serieIndex + '-' + i).attr('startOffset', d => {
          // find max value
          const maxIndex = d3.maxIndex(d, e => e[1] - e[0]); // get x position
...
function render$q(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const nest = d3.rollup(data, v => v[0], ...mapping.hierarchy.value.map(level => d => d.hierarchy.get(level)));
  const hierarchy = d3.hierarchy(nest).sum(d => d[1] instanceof Map ? 0 : d[1].size); // since maps have also a .size porperty, sum only values for leaves, and not for Maps
  ...
  const arc = d3.arc().startAngle(d => d.x0).endAngle(d => d.x1).padAngle(padding / (radius / 2)) // convert padding in radians
  ...
  }).attr('display', d => d.data[0] != '' ? '' : 'none').attr('d', arc).append('title').text(d => d.data[0]);
  ...
  if (autoHideLabels) {
    rawgraphsCore.labelsOcclusion(textGroups, d => d.size);
  }
...
function render$r(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const treemap = d3.treemap().tile(d3__namespace[tiling]).size([chartWidth, chartHeight]).padding(padding).round(true);

  if (showHierarchyLabels) {
    treemap.paddingTop(12);
  }

  const root = treemap(hierarchy); // add background

  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'viz'); // if selected, draw a rectangle for each level in the hierarchy
...
function render$s(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const yScale = d3.scaleLinear().domain(d3.extent(data, d => d.value)).nice().range([chartHeight, 0]); // prepare the data

  const nestedData = d3.rollups(data, v => ({
    group: v[0].group,
    color: v[0].color,
    bins: d3.bin().domain(yScale.domain()).thresholds(binsNumber)(v.map(e => e.value)),
  ...
  const svg = d3.select(svgNode).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('id', 'viz');
  const groupsDomain = nestedData.map(d => d[0]);
  const xScale = d3.scaleBand().range([0, chartWidth]).domain(groupsDomain).padding(padding / (chartWidth / groupsDomain.length)); // convert padding from pixel to percentage @TODO: not working, check
  // get the max value in the bins

  const maxValue = d3.max(nestedData.map(d => d[1].bins.map(e => e.length)).flat(1)); // compute the scale used to draw shapes

  const shapeScale = d3.scaleLinear().range([0, xScale.bandwidth()]).domain([-maxValue, maxValue]); // append scales

  svg.append('g').attr('id', 'y axis').call(d3.axisLeft(yScale)).append('text').attr('x', 4).attr('text-anchor', 'start').attr('dominant-baseline', 'hanging').text(mapping['value'].value).styles(styles.axisLabel);
  const xAxisG = svg.append('g').attr('id', 'x axis').attr('transform', 'translate(0,' + chartHeight + ')').call(d3.axisBottom(xScale));
  xAxisG.append('text').attr('x', chartWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['group'].value).styles(styles.axisLabel);
  ...
  }).attr('d', d3.area().x0(d => shapeScale(-d.length)).x1(d => shapeScale(d.length)).y(d => yScale((d.x0 + d.x1) / 2)).curve(d3__namespace[interpolation]));
...
function render$t(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  let voronoiTreemap = d3VoronoiTreemap.voronoiTreemap().prng(random); // set the minimum weight ratio to zero to get more proportional areas
  ...
  if (clipToPolygon) {
    ...
    let hscale = d3.scaleLinear().domain(d3.extent(points, d => d[0])).range([-chartWidth / 2, chartWidth / 2]);
    let vscale = d3.scaleLinear().domain(d3.extent(points, d => d[1])).range([-chartHeight / 2, chartHeight / 2]); // create the shape
  ...
  voronoiTreemap(hierarchy); // get nodes

  let allNodes = hierarchy.descendants().sort((a, b) => d3.ascending(a.height, b.height)); // @TODO understand how to place labels for hierarchical cells
  ...
  d3.select(svgNode).append('rect').attr('width', showLegend ? width + legendWidth : width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const svg = d3.select(svgNode).append('g').attr('id', 'viz') // translate to the center
  ...
  .attr('d', d => d3.line()(d.polygon) + 'z').attr('fill', d => d.height > 0 ? 'none' : colorScale(d.data[1].color)).attr('stroke', background).attr('stroke-width', d => (d.height + 1) * padding).attr('id', d => d.data[0]); // draw labels
...
function render$u(node, data, visualOptions, mapping, originalData, styles) {
  ...
  const xAxis = g => {
    return g.attr('transform', `translate(0,${chartHeight})`).call(d3.axisBottom(xScale)).call(g => g.append('text').attr('x', chartWidth).attr('dy', -5).attr('text-anchor', 'end').text(mapping['x'].value).styles(styles.axisLabel));
  };
...
function render$v(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  d3.select(svgNode).append('rect').attr('width', width).attr('height', height).attr('x', 0).attr('y', 0).attr('fill', background).attr('id', 'background');
  const svg = d3.select(svgNode).append('g').attr('transform', `translate(${translationX},${translationY})`).attr('id', 'chord_diagram');
  ...
  function createChords(matrix, innerRadius) {
    const chords = d3.chordDirected().padAngle(chordPadding / innerRadius).sortGroups((a, b) => {
      switch (sortNodesBy) {
  ...
  function drawRing(svg, chords, innerRadius, outerRadius) {
    ...
    svg.append('g').selectAll('g').data(chords.groups).join('g').append('path').attr('fill', d => chordColors(names[d.index])).attr('d', arc);
  }

  function drawChords(svg, chords, innerRadius) {
    const ribbon = showHeads ? d3.ribbonArrow().headRadius(headRadius) : d3.ribbon();
    ribbon.padAngle(ribbonPadding / innerRadius).sourceRadius(() => innerRadius - chordPaddingSource).targetRadius(() => innerRadius - chordPaddingTarget);
