...
import { rollups, selection, transition, select, ascending, descending, max, scaleBand, groups, sum, scaleSqrt, scaleLinear, timeFormat, extent, axisBottom, axisLeft, rollup, stack, scaleTime, forceSimulation, forceX, forceY, forceCollide, median, scalePoint, quantile, line, min, area, maxIndex, hierarchy, pack, arc, cluster, tree, linkRadial, lab, geoPath, range, scan, polygonHull, linkHorizontal, greatest, axisTop, pie, lineRadial, curveLinearClosed, curveBasisClosed, curveCardinalClosed, curveCatmullRomClosed, partition, treemap as treemap$1, selectAll, bin, randomNormal, randomLcg, Delaunay, chordDirected, ribbonArrow, ribbon } from 'd3';
...
function render$1(svgNode, data, visualOptions, mapping, originalData) {
  ...
  const sizeScale = scaleSqrt().domain([0, max(graph.nodes, d => d[nodeSize])]).range([minRadius, maxRadius]); // widthScale (for links)
...
function render$2(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, seriesIndex) {
    ...
    if (horizontalBars) {
      ...
      const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(axisBottom(barScale).tickSizeOuter(0)).call(g => g.append('text').attr('x', seriesWidth).attr('y', -4).attr('text-anchor', 'end').attr('display', seriesIndex === 0 || repeatAxesLabels ? null : 'none').text(mapping['bars'].value).styles(styles.axisLabel));
...
function render$3(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, serieIndex) {
    ...
    const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(axisBottom(setScale).tickSizeOuter(0));
...
function render$4(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, serieIndex) {
    ...
    const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + sizeScale(0) + ')').call(axisBottom(stacksScale).tickSizeOuter(0));
...
function render$6(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  const groupsWithMedian = rollups(data, v => median(v, d => d.value), d => d.group);
  const groupSortings = {
    'Name': (a, b) => ascending(a[0], b[0]),
    'Median (descending)': (a, b) => descending(a[1], b[1]),
    'Median (ascending)': (a, b) => ascending(a[1], b[1]),
    'Original': () => 0
  };
  ...
  const xAxisG = svg.append('g').attr('id', 'x axis').attr('transform', 'translate(0,' + chartHeight + ')').call(axisBottom(xScale));
...
function render$8(svgNode, data, visualOptions, mapping, originalData, styles) {
  ...
  series.each(function (d, serieIndex) {
    ...
    const xAxis = selection.append('g').attr('id', 'xAxis').attr('transform', 'translate(0,' + serieHeight + ')').call(axisBottom(xScale).tickSizeOuter(0));
